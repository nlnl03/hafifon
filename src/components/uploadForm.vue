  ...
  <div class="main">
             <q-form @submit.prevent="submitForm"  class="q-ma-auto">
              ...
              <template  v-slot:append v-if="file != null">
                <q-icon name="fas fa-times" @click.stop.prevent="file = null" class="cursor-pointer" />
              </template>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      checkFormValidity(){
        this.isFormValid = !!this.selectedWeek && !!this.selectedLesson && this.files.length > 0
      },
      ...
      async getData(){
        ...
        try{
          ...
          const promises = weeksData.map(async (week) => {
            const lessonsRes = await axios.get(this.$sharePointUrl + `getByTitle('lessons')/items?$filter=week eq ${week.ID}`)
            const lessonsData = lessonsRes.data.value;

            const lessonsWithDetails = await Promise.all(
              lessonsData.map(async (lesson) => {
                const practicesRes = await axios.get(this.$sharePointUrl + `getByTitle('practices')/items?$filter=lesson eq ${lesson.ID}`)
                const practicesData = practicesRes.data.value;

                const practicesWithDetails = await Promise.all(
                  practicesData.map(async (practice) => {
                      const practicesDataRes = await axios.get(this.$sharePointUrl + `getByTitle('practicesData')/items?$filter=practiceId eq ${practice.ID}`)
      ...
      async submitForm(){
        try{
          ...
          const lessonUpdateUrl = this.$sharePointUrl + `getByTitle('lessons')/items('${this.selectedLesson}')`
          const lessonUpdateData = {
            ...
            const lessonUpdateRes = await axios.post(
              ...
              console.log("file uploaded and lessons list updated successfully:", uploadRes, lessonUpdateRes)
        }catch(error){
    ...
    computed:{
      ...
      isFormValid:{
        get(){
          return !!this.selectedWeek && !!this.selectedLesson && this.files.length > 0;
        }
      }
    }
  };
